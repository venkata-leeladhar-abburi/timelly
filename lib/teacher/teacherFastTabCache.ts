...
const SESSION_KEY = "erp:teacher-fast-tabs:v1";
...
export function peekTeacherResource<T>(namespace: string, key = "default"): T | null {
...
export function setTeacherResource<T>(namespace: string, key: string, value: T): void {
  const fullKey = cacheKey(namespace, key);
...
export function invalidateTeacherResource(namespace: string, key?: string): void {
...
export async function loadTeacherResource<T>(
  namespace: string,
  key: string,
  loader: () => Promise<T>,
  ...
  if (!options?.revalidate) {
    const cached = peekTeacherResource<T>(namespace, key);
  ...
  const run = loader().then((value) => {
    setTeacherResource(namespace, key, value);
    return value;
  });
