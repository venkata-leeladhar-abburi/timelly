...
import { CheckCircle, Pencil, Printer, Search, Trash2, UserPlus, Loader2, IndianRupee, Download, ChevronDown } from "lucide-react";
...
import { BOARDING, GRADES, defaultForm } from "./shared/constants";
import {
  classLabel,
...
import { Select } from "./shared/PresentationalBits";
  ...
  return (
    <>
      ...
      <AdmissionFeeAssignDialog
        dialog={feeAssignDialog}
        classBaseFeeTotal={classBaseFeeTotal}
        existingStudentExtras={existingStudentExtras}
        editingExistingFeeId={editingExistingFeeId}
        editingExistingFeeName={editingExistingFeeName}
        editingExistingFeeAmount={editingExistingFeeAmount}
        editingExistingFeeSplit={editingExistingFeeSplit}
        onStartEditExisting={startEditExistingFee}
        onCancelEditExisting={cancelEditExistingFee}
        onEditingNameChange={setEditingExistingFeeName}
        onEditingAmountChange={setEditingExistingFeeAmount}
        onEditingSplitChange={setEditingExistingFeeSplit}
        onSaveExisting={updateExistingStudentFee}
        onDeleteExisting={deleteExistingStudentFee}
        catalogLoading={catalogLoading}
        dbFeeHeadOptions={dbFeeHeadOptions}
        onToggleDbFeeHeadOption={toggleDbFeeHeadOption}
        onAddSelectedDbHeads={addSelectedDbHeadsToRows}
        assignFeeError={assignFeeError}
        feeAssignRows={feeAssignRows}
        onFeeAssignRowChange={patchFeeAssignRow}
        onRemoveFeeAssignRow={removeFeeAssignRow}
        onAddFeeAssignRow={addAssignFeeRow}
        assigningFees={assigningFees}
        onCancel={() => setFeeAssignDialog(null)}
        onSave={saveAssignedFees}
      />
